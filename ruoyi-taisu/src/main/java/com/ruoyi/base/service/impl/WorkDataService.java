...
//            manWork.setIp(workBo.getIP());
        ...
        factoryBoList.forEach(factoryBo -> {
            ...
            BeanUtils.copyProperties(manFactory,manWorkFactory);
    ...
    private ManFactory instanceFactory(FactoryBo factoryBo) {
        ...
        //头像信息
        if(!StringUtils.isEmpty(factoryBo.getFace())) {
            manFactory.setFace(factoryBo.getFace());
            manFactory.setPhone(factoryBo.getPhone());
            manFactory.setAddress(factoryBo.getAddress());
        }
        return manFactory;
    }
     ...
     * @return
     */
    public String getIdCardsFromWorkBos(List<WorkBo> workBos) {
        List<String> idCardList = new ArrayList<String>();

        for (WorkBo workBo : workBos) {
            List<FactoryBo> factoryBoList = workBo.getFactoryBoList();
            for(FactoryBo factoryBo : factoryBoList){
                if(!idCardList.contains(factoryBo.getIdCard())){
                    idCardList.add(factoryBo.getIdCard());
                }
            }
        }
        return StringUtils.join(idCardList.toArray(),",");
    }
     ...
     * @return
     */
    public void setFaceFromCenter(List<WorkBo> workBos){
        String idCards = this.getIdCardsFromWorkBos(workBos);
        JSONObject param=new JSONObject();
        param.put("noFaceIdCards",idCards);
        ...
        if (!StringUtils.isEmpty(res)) {
            ...
            Map<String, JSONObject> faceMap = (Map<String, JSONObject>) resObj.get("data");

            //给数据添加头像信息
            for(WorkBo workBo : workBos){
                List<FactoryBo> factoryBoList = workBo.getFactoryBoList();
                for(FactoryBo factoryBo : factoryBoList){
                    JSONObject jObj = faceMap.get(factoryBo.getIdCard());
                    if(jObj!=null){
                        factoryBo.setFace(jObj.getString("face"));
                        factoryBo.setPhone(jObj.getString("phone"));
                        factoryBo.setAddress(jObj.getString("address"));
                    }
                }
            }
        }
