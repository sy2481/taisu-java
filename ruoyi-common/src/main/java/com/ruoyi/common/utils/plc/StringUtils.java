...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    /**
     * 根据keys获取所有的值，按照splice拼接
     * @param keys 所有的key按照,分割
     * @param tree Map树
     * @param splice 拼接符号
     * @return
     */
    public static String getParamKey(String keys, Map<String,Object> tree, String splice){
        String[] keyArray = keys.split(",");
        StringBuilder params = new StringBuilder();
        for (String key : keyArray) {
            //获取值
            String value = getNodeDataByParamKey(key, tree);
            if(StringUtils.isNotEmpty(value)){
                params.append(value)
                        .append(splice);
            }
        }
        return params.substring(0,params.length()-(splice.length()));
    }
     ...
     * @return
     */
    public static String getNodeDataByParamKey(String key,Map<String,Object> tree){
        //获取key的所有层级
        String[] keys = key.split("\\.");
        for (int i = 0; i < keys.length-1; i++) {
            if(!tree.containsKey(keys[i])){
                log.error("key不存在>>>>>>>>>>>>>>>{}，参数为>>>>>>{}",key, com.alibaba.fastjson.JSONObject.toJSONString(tree));
                return null;
            }else{
                Object data = tree.get(keys[i]);
        ...
        Object o = tree.get(keys[keys.length - 1]);
        if(o instanceof String){//如果是String直接返回
            return (String) o;
        }else{//如果是对象返回json字符串
